...
BOOST_AUTO_TEST_CASE(findearliestatleast_test)
{
    ...
    for (unsigned int i=0; i<vBlocksMain.size(); i++) {
        ...
        vBlocksMain[i].BuildSkip();
        if (i < 10) {
            vBlocksMain[i].blockHeader.nTime = i;
            vBlocksMain[i].nTimeMax = i;
        } else {
            // randomly choose something in the range [MTP, MTP*2]
            int64_t medianTimePast = vBlocksMain[i].GetMedianTimePast();
            int r = insecure_rand() % medianTimePast;
            vBlocksMain[i].blockHeader.nTime = r + medianTimePast;
            vBlocksMain[i].nTimeMax = std::max(vBlocksMain[i].blockHeader.nTime, vBlocksMain[i-1].nTimeMax);
        }
    }
    // Check that we set nTimeMax up correctly.
    unsigned int curTimeMax = 0;
    for (unsigned int i=0; i<vBlocksMain.size(); ++i) {
        curTimeMax = std::max(curTimeMax, vBlocksMain[i].blockHeader.nTime);
    ...
    for (unsigned int i=0; i<10000; ++i) {
        ...
        int64_t test_time = vBlocksMain[r].blockHeader.nTime;
